...
namespace
{
	...
	[[nodiscard]]
	server::service::PeerSessionService::AuthenticatedIdentity
		MakeAuthenticatedIdentity() noexcept
	{
	...
	)
	{
		...
		tests::Expect(
			result,
			joinResult.persistentPlayerId == testPersistentPlayerId,
		...
		if (peerState != nullptr)
		{
			tests::Expect(
				...
				"PeerSessionService: join endpoint key"
			);
			...
			tests::Expect(
				...
				"PeerSessionService: join persistent player id stored"
			);
			...
			tests::Expect(
				...
				"PeerSessionService: join last recv time"
			);
			...
			tests::Expect(
				result,
				peerRoomManager.FindJoinedPeerByAccountId(9999)
				== nullptr,
			...
			tests::Expect(
				result,
				peerRoomManager.FindJoinedPeerByAccountId(0)
				== nullptr,
	...
	)
	{
		...
		const server::service::PeerSessionService::JoinResult
			firstJoinResult =
			JoinPeerForTest(
				service,
				endpointKey,
		...
		if (peerState != nullptr)
		{
			tests::Expect(
				...
				"PeerSessionService: existing join identity preserved"
			);
	...
	)
	{
		...
		tests::Expect(
			...
			"PeerSessionService: existing join current state no broadcast"
		);

		tests::Expect(
			result,
			retryJoinResult.playerId == firstJoinResult.playerId,
			"PeerSessionService: existing join preserves player id"
		);

		tests::Expect(
			result,
			retryJoinResult.roomId == 2,
			"PeerSessionService: existing join returns current room"
		);
		...
		tests::Expect(
			result,
			peerRoomManager.GetRoomMemberCount(1) == 0,
			"PeerSessionService: existing join initial room remains empty"
		);

		tests::Expect(
			result,
			peerRoomManager.GetRoomMemberCount(2) == 1,
			"PeerSessionService: existing join current room count"
		);
	...
	)
	{
		...
		gameWorld.AddBullet(
			server::game::BulletState{
				.bulletId = gameWorld.AllocateBulletId(),
				.roomId = 1,
			}
			);

		const server::service::PeerSessionService::LeaveResult
			leaveResult =
			service.LeavePeer(
				endpointKey,
				peerRoomManager,
		...
		tests::Expect(
			...
			== testPersistentPlayerId,
			"PeerSessionService: leave persistent player id"
		);

		tests::Expect(
			result,
			leaveResult.roomId == 1,
		...
		tests::Expect(
			...
			"PeerSessionService: leave clears empty room transient state"
		);
	}
	...
	void RunChangePeerRoomTest(tests::DebugTestResult& result)
	{
		...
		tests::Expect(
			result,
			changeResult.persistentPlayerId
			== testPersistentPlayerId,
			"PeerSessionService: room change persistent player id"
		);

		tests::Expect(
			result,
			changeResult.previousRoomId == 1,
			"PeerSessionService: room change previous room"
		);

		tests::Expect(
			result,
			changeResult.nextRoomId == 2,
		...
		tests::Expect(
			result,
			gameWorld.GetBulletCount() == 0,
			"PeerSessionService: room change clears previous empty room transient state"
		);
	...
	void RunTimedOutPeerBecomesRecoverableAndStopsInputTest(
		tests::DebugTestResult& result
	)
	{
		server::service::PeerSessionService service;
		...
		tests::Expect(
			result,
			playerState != nullptr,
			"PeerSessionService: recoverable player exists before timeout"
		);
		...
		const server::service::PeerSessionService::RecoverablePeerList
			recoverablePeerList =
		...
		tests::Expect(
			...
			"PeerSessionService: timed out player becomes recoverable"
		);

		tests::Expect(
			result,
			peerRoomManager.GetPeerCount() == 1,
			"PeerSessionService: recoverable peer remains stored"
		);

		tests::Expect(
			result,
			peerRoomManager.GetRoomMemberCount(1) == 1,
			"PeerSessionService: recoverable peer remains room member"
		);
		...
		tests::Expect(
			result,
			peerState != nullptr,
			"PeerSessionService: recoverable peer remains joined"
		);
		...
		if (peerState != nullptr)
		{
			tests::Expect(
				...
				"PeerSessionService: peer connection becomes recoverable"
			);
		}
		...
		tests::Expect(
			result,
			playerState != nullptr,
			"PeerSessionService: recoverable player remains in world"
		);
		...
		if (playerState != nullptr)
		{
			tests::Expect(
				...
				playerState->inputFlags
				== common::game::InputFlags::None,
				"PeerSessionService: recoverable player input stopped"
			);
		}
		...
		tests::Expect(
			...
			"PeerSessionService: recoverable room transient state preserved"
		);

		if (recoverablePeerList.size() == 1)
		{
			const server::service::PeerSessionService::RecoverablePeer&
				recoverablePeer =
			...
			tests::Expect(
				...
				"PeerSessionService: recoverable endpoint"
			);

			tests::Expect(
				result,
				recoverablePeer.playerId == joinResult.playerId,
				"PeerSessionService: recoverable player id"
			);

			tests::Expect(
				result,
				recoverablePeer.persistentPlayerId
				== testPersistentPlayerId,
				"PeerSessionService: recoverable persistent player id"
			);

			tests::Expect(
				result,
				recoverablePeer.roomId == 1,
				"PeerSessionService: recoverable room id"
			);
		}
	}

	void RunExpiredRecoverablePeerRemovesPlayerTest(
		tests::DebugTestResult& result
	)
	{
		server::service::PeerSessionService service;
		...
		tests::Expect(
			result,
			recoverablePeerList.size() == 1,
			"PeerSessionService: expired setup peer becomes recoverable"
		);

		const server::service::PeerSessionService::ExpiredRecoverablePeerList
			expiredPeerList =
			service.RemoveExpiredRecoverablePeers(
				recoverableTime
		...
		tests::Expect(
			...
			"PeerSessionService: recoverable peer expires"
		);

		tests::Expect(
			result,
			peerRoomManager.GetPeerCount() == 0,
			"PeerSessionService: expired recoverable peer removed"
		);

		tests::Expect(
			result,
			peerRoomManager.GetRoomMemberCount(1) == 0,
			"PeerSessionService: expired recoverable peer removed from room"
		);

		tests::Expect(
			result,
			gameWorld.FindPlayer(joinResult.playerId) == nullptr,
			"PeerSessionService: expired recoverable player removed"
		);

		tests::Expect(
			result,
			gameWorld.GetPlayerCount() == 0,
			"PeerSessionService: expired recoverable player count"
		);

		tests::Expect(
			result,
			gameWorld.GetBulletCount() == 0,
			"PeerSessionService: expired recoverable peer clears empty room transient state"
		);

		if (expiredPeerList.size() == 1)
		{
			const server::service::PeerSessionService::
				ExpiredRecoverablePeer& expiredPeer =
				expiredPeerList.front();

			tests::Expect(
				result,
				expiredPeer.endpointKey == endpointKey,
				"PeerSessionService: expired endpoint"
			);

			tests::Expect(
				result,
				expiredPeer.playerId == joinResult.playerId,
				"PeerSessionService: expired player id"
			);

			tests::Expect(
				result,
				expiredPeer.persistentPlayerId
				== testPersistentPlayerId,
				"PeerSessionService: expired persistent player id"
			);

			tests::Expect(
				result,
				expiredPeer.roomId == 1,
				"PeerSessionService: expired room id"
			);
		}
	}

	void RunJoinAuthenticatedPeerTest(
		tests::DebugTestResult& result
	)
	{
		server::service::PeerSessionService service;
		...
		tests::Expect(
			result,
			registered,
			"PeerSessionService: authenticated account registered"
		);

		const server::service::PeerSessionService::
			JoinAuthenticatedPeerResult joinResult =
		...
		tests::Expect(
			...
			JoinAuthenticatedPeerStatus::Joined,
			"PeerSessionService: authenticated peer joined"
		);

		tests::Expect(
			result,
			joinResult.joinResult.shouldSendResponse,
			"PeerSessionService: authenticated join sends response"
		);

		tests::Expect(
			result,
			joinResult.joinResult.shouldBroadcastPlayerJoined,
			"PeerSessionService: authenticated join broadcasts"
		);

		tests::Expect(
			result,
			authenticatedAccountRegistry.GetCount() == 0,
			"PeerSessionService: authenticated account consumed"
		);

		tests::Expect(
			result,
			peerRoomManager.GetJoinedPeerCount() == 1,
			"PeerSessionService: authenticated join creates peer"
		);

		tests::Expect(
			result,
			gameWorld.GetPlayerCount() == 1,
			"PeerSessionService: authenticated join creates player"
		);
	}

	void RunJoinAuthenticatedExistingPeerTest(
		tests::DebugTestResult& result
	)
	{
		server::service::PeerSessionService service;
		...
		const server::service::PeerSessionService::
			JoinAuthenticatedPeerResult firstResult =
			service.JoinAuthenticatedPeer(
				endpointKey,
				testSessionToken,
				1,
		...
		const server::service::PeerSessionService::
			JoinAuthenticatedPeerResult retryResult =
			service.JoinAuthenticatedPeer(
				endpointKey,
				testSessionToken,
				1,
		...
		tests::Expect(
			...
			retryResult.status
			== server::service::PeerSessionService::
			JoinAuthenticatedPeerStatus::ExistingPeer,
			"PeerSessionService: authenticated existing peer detected"
		);

		tests::Expect(
			result,
			retryResult.joinResult.shouldSendResponse,
			"PeerSessionService: existing peer retry sends response"
		);

		tests::Expect(
			result,
			!retryResult.joinResult.shouldBroadcastPlayerJoined,
			"PeerSessionService: existing peer retry no broadcast"
		);
	}
	...
	)
	{
		...
		tests::Expect(
			result,
			peerRoomManager.GetPeerCount() == 0,
			"PeerSessionService: unauthenticated join creates no peer"
		);

		tests::Expect(
			result,
			gameWorld.GetPlayerCount() == 0,
			"PeerSessionService: unauthenticated join creates no player"
		);
	}

	void RunJoinAuthenticatedExistingPeerRejectsTokenMismatchTest(
		tests::DebugTestResult& result
	)
	{
		server::service::PeerSessionService service;
		...
		tests::Expect(
			...
			retryResult.status
			== server::service::PeerSessionService::
			JoinAuthenticatedPeerStatus::Rejected,
...
namespace tests::server
{
	DebugTestResult RunPeerSessionServiceTests()
	{
		...
		RunTimedOutPeerBecomesRecoverableAndStopsInputTest(result);
		RunExpiredRecoverablePeerRemovesPlayerTest(result);
